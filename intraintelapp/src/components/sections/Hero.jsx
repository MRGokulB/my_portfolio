...
export default function Hero({
  ...
  const isInView = useInView(containerRef); // Reusing containerRef as it wraps the visual area

  useEffect(() => {
    if (videoRef.current) {
      if (isInView) {
        videoRef.current.play().catch(() => { /* Auto-play restrictions */ });
      } else {
              ...
              >
                <div className="relative group cursor-pointer" onClick={() => setIsVideoOpen(true)}>
                    ...
                    <div className="relative aspect-video bg-slate-100 w-full overflow-hidden">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center z-10 opacity-0 group-hover:opacity-100 transition-all duration-500">
                        <motion.button
                          ...
                          className="w-16 h-16 rounded-full bg-white/20 backdrop-blur-md border border-blue-300/40 shadow-xl flex items-center justify-center text-white group-hover:bg-white/30 transition-all duration-300"
                        >
                          <PlayCircle size={32} strokeWidth={1.5} className="ml-1 drop-shadow-md" />
                  ...
                  <motion.div
                    ...
                    className="absolute right-2 sm:-right-4 top-1/2 -translate-y-1/2 bg-white/95 backdrop-blur shadow-lg border border-slate-100 rounded-lg p-2.5 flex items-center gap-2.5 z-30 transform scale-90 sm:scale-100"
                  >
                    <div className="w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center text-blue-600">
                      <Shield size={16} />
                    </div>
                    <div>
                      <div className="text-[11px] font-bold text-slate-900 leading-tight">Secure</div>
                      <div className="text-[9px] text-slate-500 font-medium">Enterprise Ready</div>
                    </div>
                  </motion.div>
                </div>

                {/* Caption with Live Pulse */}
