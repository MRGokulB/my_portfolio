...
import { ArrowLeft, ChefHat, Sparkles, LogOut, Volume2, VolumeX } from 'lucide-react';
...
const DashboardOverview = () => {
    const { subscribers, sales, dailyMenu, addSubscriber, updateMenu, adminAudioEnabled, setAdminAudioEnabled } = useAdmin();
        ...
        >
            <motion.header variants={itemVariants} className="flex justify-between items-center">
                ...
                <div className="flex gap-3 items-center">
                    <button
                        onClick={() => {
                            const newState = !adminAudioEnabled;
                            setAdminAudioEnabled(newState);
                            localStorage.setItem('adminAudioEnabled', newState);
                            if (newState) {
                                new Audio('/notification.mp3').play();
                            }
                        }}
                        className={`p-3 rounded-xl transition-colors ${adminAudioEnabled ? 'bg-primary-100 text-primary-600' : 'bg-gray-100 text-gray-400'}`}
                        title={adminAudioEnabled ? "Mute Alerts" : "Enable Alerts"}
                    >
                        {adminAudioEnabled ? <Volume2 size={24} /> : <VolumeX size={24} />}
                    </button>
